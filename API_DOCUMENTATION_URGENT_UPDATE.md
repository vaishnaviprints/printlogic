...
**Base URL:** `https://printify-app.preview.emergentagent.com/api`
...
```bash
# Login
ADMIN_TOKEN=$(curl -s -X POST "https://printify-app.preview.emergentagent.com/api/auth/login" \
...
```bash
# Login
VENDOR_TOKEN=$(curl -s -X POST "https://printify-app.preview.emergentagent.com/api/auth/vendor/login" \
  -F "email=central@vaishnavi.com" \
  -F "password=vendor123" | jq -r '.access_token')

# Accept order
curl -X PUT "https://printify-app.preview.emergentagent.com/api/vendor/orders/order_abc123/action" \
  -H "Authorization: Bearer $VENDOR_TOKEN" \
  -F "action=accept" \
...
### Toggle Store
```bash
curl -X PUT "https://printify-app.preview.emergentagent.com/api/vendor/store" \
  -H "Authorization: Bearer $VENDOR_TOKEN" \
  -F "store_open=false"
```
...
**https://printify-app.preview.emergentagent.com/api**
